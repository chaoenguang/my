class Banner {
  constructor (id) {
    ...
    this.img = this.ele.querySelectorAll('ul>li img')
    this.zy = this.ele.querySelectorAll('.leftRight>span')
    // 获取左右切换按钮
  ...
  overOut () {
    ...
    this.img.forEach(item=>{
      item.addEventListener('mouseover', () => clearInterval(this.timerLoop))
      item.addEventListener('mouseout', () => this.autoplay())
    })
    this.zy.forEach(item=>{
      item.addEventListener('mouseover', () => clearInterval(this.timerLoop))
      item.addEventListener('mouseout', () => this.autoplay())
    })

    // this.img.addEventListener('mouseover', () => clearInterval(this.timerLoop))
    // this.img.addEventListener('mouseout', () => this.autoplay())
  }
  // 4. 左右切换
  leftRight () {
    // 事件委托
